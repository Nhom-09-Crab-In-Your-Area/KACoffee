...
function Add_new_order() {
    ...
    name_customer[l-1].innerHTML = document.getElementById("customer-name").value
...
function get_info(j) {
    ...
    cost = String(cost)
    prd_cost.innerHTML = "Giá : " + cost + " VNĐ"
    ...
    var a_new_product = new a_product(myJSON[j]._id, size[j].value, sugar[j].value, ice[j].value, number[j].value)
